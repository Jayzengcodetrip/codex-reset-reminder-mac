...
- Keep every independently pending announcement and its own seconds-level countdown. For a date-only announcement, show 23:59 as the final minute of its Los Angeles day and count down until the following 00:00:00; label this an estimate, not an official exact time. While pending, show the live Los Angeles weekday and 24-hour `HH:mm:ss` without month or day, even after the estimate passes. Beijing shows the corresponding weekday only. Read/unread state or elapsed target time alone must not remove one; use explicitly associated source completion or cancellation. Do not assign ordinal numbers to announcements.
