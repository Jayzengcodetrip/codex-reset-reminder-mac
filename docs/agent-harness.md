...
CodexNotch uses a lightweight harness with executable acceptance checks. It is a native macOS app with no online service or database migration. GitHub Actions runs verification on source changes and creates release assets from version tags. The harness prevents fixes to notch visuals or the state machine from overwriting confirmed user experience, privacy boundaries, and packaging behavior.
...
Local `full` verification does not send notifications, request real task operations, or change remote state by default. Only the local `dist/` packaging directory is updated, and that directory is ignored by Git. The separate GitHub release workflow uploads archives only after tag-triggered checks pass.
...
| Required CI check | GitHub Actions runs contracts, XCTest, release build, app bundle and self-check validation |
| Deployment target | GitHub Releases receives DMG, ZIP and SHA-256 assets after a matching version tag passes release checks |
