...
final class NotchWindowController: NSWindowController {
    ...
    private let panelRestoration = NotchPanelRestoration()
    ...
    private func observeScreenChanges() {
        screenObserver = NotificationCenter.default.addObserver(
            ...
            // Screen mode restoration can continue after the first notification.
            // A hidden display still needs its transparent hover entry restored.
            self.reassertNotchPanelAfterApplicationSwitch(displayIsEnabled: false)
        }
    }

    func reassertNotchPanelAfterApplicationSwitch(displayIsEnabled: Bool) {
        // An application or Space transition may finish withdrawing windows
        // after the activation callback. Retry only during that short transition.
        panelRestoration.request { [weak self] in
            guard let self else { return }
            self.onScreenParametersChanged?()
