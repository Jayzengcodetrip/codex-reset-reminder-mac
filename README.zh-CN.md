...
| 你想知道的 | 应用显示的内容 |
| --- | --- |
| 临时重置什么时候来 | 对只公布日期或星期的预告，按洛杉矶当地预告日 **23:59** 这一最后一分钟估算，显示秒级倒计时；同时显示洛杉矶当前**周几 HH:mm:ss**。北京时间只显示对应的星期，不显示预计钟点。有两条尚未完成的预告时，两条分别倒数；没有可推定日期时，明确显示时间待公布。 |
...
提醒依据 [NextReset](https://nextreset.net/) 的公开数据，并非直接读取 X 的付费实时 API。**提醒速度取决于站方何时捕获和发布消息、网络连接以及本机轮询时间；无法保证 Tibo 发布后五分钟内必达。**洛杉矶是应用使用的参考时区，并非官方指定的重置时区；对日期级预告使用的 23:59 是估算边界，并非 Tibo 公布的精确执行时间。若接口暂时不可用，应用会保留已取得的记录并提示检查失败，不会把旧记录伪装成刚刚确认的新预告。
...
- **临时重置预告**：只公布当地日期或星期时，按该日最后一分钟（洛杉矶 23:59）估算，秒级倒计时到当地次日 00:00:00 结束。随后显示“预计时间已过 · 等待确认”，而不是宣布重置完成。卡片在预告待确认期间始终显示“洛杉矶现在：周几 HH:mm:ss”，不显示月、日；北京时间只给对应的星期。若原帖明确给出钟点，则按明确时刻倒数。只有与该预告关联的明确完成或取消消息才结束待确认提醒；点开、标记已读、时间经过或账户每周刷新都不会自动结束。
