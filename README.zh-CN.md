...
| 你想知道的 | 应用显示的内容 |
| --- | --- |
| 临时重置什么时候来 | 对只公布日期或星期（如“周二”）的预告，先按洛杉矶当地时间倒数到**周二 00:00:00**，即最早可能窗口的开始；进入周二后，改为倒数这一天剩余的时间，直到**周三 00:00:00**。此后不显示倒计时，继续等待确认。全程显示洛杉矶当前**周几 HH:mm:ss**；倒计时阶段的北京时间只显示对应截止的星期，不显示预计钟点。有两条尚未完成的预告时分别计算；没有可推定日期时明确显示时间待公布。 |
...
提醒依据 [NextReset](https://nextreset.net/) 的公开数据，并非直接读取 X 的付费实时 API。**提醒速度取决于站方何时捕获和发布消息、网络连接以及本机轮询时间；无法保证 Tibo 发布后五分钟内必达。**洛杉矶是应用使用的参考时区，并非官方指定的重置时区；日期级预告的开始与结束边界都是估算，不是 Tibo 公布的精确执行时间。第一段倒计时以原帖发出时间为起点；即使接口稍后才提供该帖，应用也按当前所在阶段显示，不会重设发帖时间。若接口暂时不可用，应用会保留已取得的记录并提示检查失败，不会把旧记录伪装成刚刚确认的新预告。
...
- **临时重置预告**：只公布当地日期或星期（如“周二”）时分三段显示：① 从 Tibo 发帖时起倒数至洛杉矶周二 `00:00:00`，这是**最早可能窗口的开始**，周一 `23:59:59` 还剩 1 秒；② 周二全天重新倒数至周三 `00:00:00`，周二 `23:59:59` 还剩 1 秒；③ 若周三 `00:00:00` 仍未确认重置，停止倒计时，显示“预告日已过 · 等待确认”和洛杉矶当前时间。两个边界都是日期估算，不是官方公布的重置钟点。**两段倒计时都不意味着建议提前用尽额度：如果重置推迟，用完额度可能使你在等待期间无额度可用。**卡片和详情在预告待确认期间始终实时显示“洛杉矶现在：周几 HH:mm:ss”，不显示月、日；macOS 通知里的固定时间标为“通知时洛杉矶”，不会伪装成实时钟。倒计时阶段的北京时间只给对应截止的星期。若原帖明确给出钟点，则按明确时刻倒数。只有与该预告关联的明确完成或取消消息才结束待确认提醒；点开、标记已读、时间经过或账户每周刷新都不会自动结束。
